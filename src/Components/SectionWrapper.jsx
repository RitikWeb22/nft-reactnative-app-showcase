...
const SectionWrapper = ({title,description,showBtn,mockupImg ,banner,reverse}) => {
  return (
    <div className={`min-h-screen ${styles.section} ${reverse ? styles.bgWhite : styles.bgPrimary} ${banner}`}>
      <div className={`flex items-center ${reverse ?styles.boxReverseClass : styles.boxClass} w-11/12 w-full minmd:w-3/4`}>
      ...
      {showBtn &&(
        <Button assetUrl={assets.expo} link=""/>
      )}
        </div>
        <div className={`flex-1 ${styles.flexCenter} p-8 sm:px-0`}>
     <img src={mockupImg} className= {` ${reverse ? "fadeLeftMini" : "fadeRightMini"} {styles.sectionImg}`}/>
        </div>
      </div>
    </div>
  )
